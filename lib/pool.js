...
function Pool(options) {
  ...
  this._subscribedToSessions = false;
  ...
  this.on('peerinv', function peerGetData(peer, inv) {
    for (let el of inv.inventory) {
      if (el.type == 3) {
        if (this._candidatesMap[el.hash] === undefined && this._mySessions.length > 0) {
          var message = this._message.GetData.forDandelionEncCand(el.hash);
          peer.sendMessage(message);
        }
      }
      if (el.type == 5 && this._subscribedToSessions) {
      ...
      if (el.type == 6 && this._subscribedToSessions) {
        if (this._sessionMap[el.hash] === undefined) {
...
Pool.prototype.subscribeToSessions = function (subscribe = true) {
  this._subscribedToSessions = subscribe;
}
...
Pool.prototype._removeConnectedPeer = function _removeConnectedPeer(addr) {
  if (this._connectedPeers[addr.hash] && this._connectedPeers[addr.hash].status !== Peer.STATUS.DISCONNECTED) {
...
Pool.prototype.filterSessions = function() {
