...
var Net = require('net');
var Socks5Client = require('@aguycalled/socks5-client');
...
Peer.prototype.connect = function() {
  ...
  if (typeof navigator != 'undefined' && navigator.product == 'ReactNative') {
    this.socket.connect({port: this.port, host: this.host},function(ev) {
...
Peer.prototype._addSocketEventHandlers = function() {
  ...
  this.socket.on('close', self.disconnect.bind(this));
...
Peer.prototype.sendMessage = function(message) {
  if (this.status == Peer.STATUS.CONNECTED || this.status == Peer.STATUS.READY)
...
Peer.prototype._sendVersion = function() {
  ...
  var message = this.messages.Version({relay: this.relay, version: 80021});
